...
def test_single_volume(image_, label_, net0, net1, net2, classes, patch_size, test_save_path=None, case=None, origin=None, spacing=None):   # patch_size: [256, 256]
    ...
    if len(image.shape) == 3:
        ...
        # z_num = np.ceil(z / patch_size[0]).astype(int)
        ...
        # print('z_num, x_num, y_num:', z_num, x_num, y_num)

        # construct step matrix
        # if z_num != 1:
            # step_z = z - (z_num - 1) * patch_size[0]
                ...
                for r in range(y_num):
                    for c in range(x_num):
                        ...
                        input = torch.from_numpy(slice).unsqueeze(0).unsqueeze(
                            0).float().cuda()
                        # print('input shape:', input.shape)  # ([1, 1, 224, 224])
