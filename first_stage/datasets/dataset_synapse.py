...
class Synapse_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            img_path = os.path.join(self.data_dir + '/prep_imagesTr', slice_name)
            image = sitk.ReadImage(img_path)
            label_path = os.path.join(self.data_dir + '/prep_labelsTr', slice_name)
        ...
        # label = make_one_hot_3d(label, self.num_classes)
        # print(np.min(label), np.max(label))            # torch.Size([26, 128, 128, 128])   -1.0 24.0
        # sample['label'] = label
