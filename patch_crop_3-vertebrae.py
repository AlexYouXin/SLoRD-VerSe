...
parser.add_argument('--img_size', type=int, default=[112, 128, 96], help='input patch size of network input')
...
if __name__ == "__main__":
    root_folder = 'xxxxxxxxx/medical data/verse19_resample/'
    ...
    # SVD
    mat = np.load('xxxxxxxxx/verse19_distance_final_5.npy')
    mat = mat / max_dist
    ...
    fields = ["Name", "center", "coefficient"]
        ...
        if space_array[2] == 1.0:
            ...
            with open('name.txt', 'a') as file1, open('output.csv', 'a', newline='') as file2:  # 'a'
                for label_index in unique_labels:
                    if label_index == unique_labels[0] or label_index == unique_labels[-1]:
                        continue
                    ...
                    if label_index - 1 in unique_labels:
                        label_[label_crop == label_index - 1] = label_index - 1
                    if label_index + 1 in unique_labels:
                        label_[label_crop == label_index + 1] = label_index + 1
                    ...
                    sitk.WriteImage(image_crop, 'xxxxxxxx/verse19_patch/3_label/img/' + img_file_name.replace('_ct.nii.gz', '-{}_ct.nii.gz').format(label_index))
                    sitk.WriteImage(label_, 'xxxxxxxxx/verse19_patch/3_label/label/' + img_file_name.replace('_ct.nii.gz', '-{}_ct.nii.gz').format(label_index))
